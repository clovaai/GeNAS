...
from .config_utils import load_config as load_dataset_config
...
def get_datasets(name, root, cutout, rand_seed, byol_aug_type=None, random_label=True):
  ...
  if name == 'cifar10':
    if random_label:
      ...
      train_data = datasets.CIFAR10(root=root, train=True, transform=train_transform, download=True)
    ...
    if random_label:
      ...
      train_data = datasets.CIFAR100(root=root, train=True , transform=train_transform, download=True)
    test_data  = datasets.CIFAR100(root=root, train=True, transform=test_transform , download=True)
    assert len(train_data) == 50000 and len(test_data) == 50000
  elif name.startswith('imagenet-1k'):
    ...
    if random_label:
      ...
      train_data = ImageNet16(root=root, train=True , transform=train_transform, use_num_of_class_only=120)
    ...
    if random_label:
      ...
      train_data = ImageNet16(root=root, train=True , transform=train_transform, use_num_of_class_only=150)
    ...
    if random_label:
      ...
      train_data = ImageNet16(root=root, train=True , transform=train_transform, use_num_of_class_only=200)
...
def get_nas_search_loaders(train_data, valid_data, dataset, config_root, batch_size, workers, use_valid_no_shuffle=False):
  # NOTE: detailed dataset configuration is given in NAS-BENCH-201 paper, https://arxiv.org/pdf/2001.00326.pdf.
  ...
  if dataset == 'cifar10' or dataset == 'cifar100':
    ...
    if dataset == 'cifar10':
      cifar_split = load_dataset_config('{:}/cifar-split.txt'.format(config_root), None, None)
    elif dataset == 'cifar100':
      cifar_split = load_dataset_config('{:}/cifar100-split.txt'.format(config_root), None, None)
    ...
    if use_valid_no_shuffle:
      # NOTE: using validation dataset
      valid_loader_no_shuffle = torch.utils.data.DataLoader(torch.utils.data.Subset(xvalid_data, valid_split), batch_size=test_batch, shuffle=False, num_workers=workers, pin_memory=True)
      # NOTE: using search training dataset
      # valid_loader_no_shuffle = torch.utils.data.DataLoader(search_data, batch_size=test_batch, shuffle=False , num_workers=workers, pin_memory=True)
  elif dataset == 'ImageNet16-120':
    imagenet_test_split = load_dataset_config('{:}/imagenet-16-120-test-split.txt'.format(config_root), None, None)
    ...
    if use_valid_no_shuffle:
      # NOTE: using validation dataset
      valid_loader_no_shuffle = torch.utils.data.DataLoader(torch.utils.data.Subset(valid_data, imagenet_test_split.xvalid), batch_size=test_batch, shuffle=False, num_workers=workers, pin_memory=True)
      # NOTE: using search training dataset
      # valid_loader_no_shuffle = torch.utils.data.DataLoader(search_data, batch_size=test_batch, shuffle=False , num_workers=workers, pin_memory=True)
  else:
  ...
  if use_valid_no_shuffle:
    return search_loader, train_loader, valid_loader, valid_loader_no_shuffle
  else:
